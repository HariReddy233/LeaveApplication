...
import { sendFullApprovalEmails, sendRejectedEmployeeEmail } from "./LeaveNotificationService.js";

const updateLeaveBalanceForFinalApproval = async (leave) => {
  ...
  if (leaveTypeResult.rows.length > 0) {
    defaultBalance = leaveTypeResult.rows[0].max_days || 0;
  }
...
const notifyFinalApprovalToTeamMembers = async (leave) => {
  const teamMembersResult = await database.query(
    `WITH target_team AS (
       SELECT COALESCE(e.team, u.department) AS team_key
       FROM employees e
     ...
     FROM target_team t
     JOIN employees e ON COALESCE(e.team, (SELECT department FROM users WHERE user_id = e.user_id)) = t.team_key
  ...
  teamMembersResult.rows.forEach((member) => {
    ...
    sseService.sendToUser(member.user_id.toString(), {
      type: 'leave_status_update',
      message: `${leave.employee_name || 'Employee'} leave has been fully approved`,
      leaveId: leave.id,
      status: 'Approved',
...
export const DeleteLeaveService = async (Request) => {
  ...
  const isFullyApproved = hodStatus === 'approved' && adminStatus === 'approved';
  ...
  // If leave was fully approved and balance was deducted, restore the balance before deleting
  if (isAdmin && isFullyApproved) {
    try {
      const year = new Date(leave.start_date).getFullYear();
      // Restore leave balance by subtracting the used days and recalculating remaining
      await database.query(
        `UPDATE leave_balance 
         SET used_balance = GREATEST(0, used_balance - $1),
             remaining_balance = LEAST(total_balance, GREATEST(0, total_balance - GREATEST(0, used_balance - $1))),
             updated_at = NOW()
         WHERE employee_id = $2 
         AND leave_type = $3 
         AND year = $4`,
...
export const ApproveLeaveHodService = async (Request) => {
  ...
  const result = await database.query(
    `UPDATE leave_applications 
     SET hod_status = $1::text, 
         hod_remark = $2::text, 
         approved_by_hod = $3::integer, 
         hod_approved_at = CASE WHEN $5::boolean THEN NOW() ELSE hod_approved_at END,
         admin_deadline = CASE WHEN $5::boolean THEN (NOW() + INTERVAL '24 hours') ELSE NULL END,
         auto_approved_by_system = CASE WHEN $5::boolean THEN FALSE ELSE auto_approved_by_system END,
         updated_at = NOW()
     WHERE id = $4::integer AND hod_status = 'Pending'
     RETURNING *`,
    [status, hodRemark, approvedByHod, parseInt(id), isApproved]
  );

  if (result.rows.length === 0) {
    throw CreateError('Leave could not be updated because HOD status was changed by another process', 409);
  }
  ...
  if (finalStatus === 'Approved' && (leave.status || '').toLowerCase() !== 'approved') {
    try {
      await updateLeaveBalanceForFinalApproval(leave);
      console.log(`✅ Leave balance updated for ${leave.leave_type} (${leave.number_of_days} days) - Final approval`);
    } catch (balanceError) {
      console.error(`❌ Failed to update leave balance for leave ${id}:`, balanceError.message);
    }
  }
  ...
  if (finalStatus === 'Approved') {
    await sendFullApprovalEmails(id);
  } else if (finalStatus === 'Rejected') {
    await sendRejectedEmployeeEmail(id, approverName, comment || 'Rejected by approver');
  }
  ...
  try {
    ...
    // On final approval, notify all active team members explicitly
    if (finalStatus === 'Approved') {
      await notifyFinalApprovalToTeamMembers(leave);
    }
...
export const ApproveLeaveAdminService = async (Request) => {
  ...
  if ((leave.admin_status || 'Pending') !== 'Pending') {
    throw CreateError(`Admin action already completed with status ${leave.admin_status}`, 409);
  }
  ...
  const result = await database.query(
    `UPDATE leave_applications 
     SET admin_status = $1::text, 
         ...
         updated_at = NOW()
     WHERE id = $4::integer AND admin_status = 'Pending'
     RETURNING *`,
    [status, adminRemark, approvedByAdmin, parseInt(id)]
  );

  if (result.rows.length === 0) {
    throw CreateError('Leave could not be updated because admin status was changed by another process', 409);
  }
...
export const CalculateWorkingDaysService = async (Request) => {
  ...
  if (!workingDays || workingDays === 0) {
    throw CreateError("No working days found between the selected dates", 400);
  }
 ...
 * Returns non-working days count (weekends + location-specific holidays)
 */
export const CalculateCompOffDaysService = async (Request) => {
  ...
  const validation = await validateCompOffDates(start_date, end_date, employeeLocation, database);

  return {
    comp_off_days: validation.nonWorkingDays,
    start_date,
    end_date,
    employee_location: employeeLocation,
    valid_dates: validation.isValid,
    invalid_dates: validation.invalidDates,
  };
};

/**
 * Get Leave Reports (with filters)
 * Admin: all employees, HOD: scoped employees
 */
export const GetLeaveReportsService = async (Request) => {
  ...
  if (!isAdmin && EmployeeId) {
    query += ` AND (
      ...
      OR (e.manager_id IS NULL AND EXISTS (
        SELECT 1 FROM users hod_u
        JOIN employees hod_e ON hod_e.user_id = hod_u.user_id
