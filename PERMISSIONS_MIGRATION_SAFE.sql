...
-- Step 1: Backup existing user_permissions if it exists
DO $$
BEGIN
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'user_permissions') THEN
        -- Create backup table
        CREATE TABLE IF NOT EXISTS user_permissions_backup AS 
        SELECT * FROM user_permissions;
        RAISE NOTICE 'Backed up existing user_permissions to user_permissions_backup';
    END IF;
END $$;

-- Step 2: Drop existing user_permissions table if structure is wrong
DO $$
BEGIN
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'user_permissions') THEN
        ...
        IF NOT EXISTS (
            ...
            RAISE NOTICE 'Dropped existing user_permissions table (incorrect structure)';
        END IF;
    END IF;
END $$;

-- Step 3: Ensure permissions table exists
...
-- Step 5: Create user_permissions table with correct structure
...
UNION ALL
SELECT 
    'User permissions table' as table_name,
    COUNT(*) as record_count 
