...
export default function LeaveListTable({
  ...
  showActions = true,
  canEdit = (record) => {
    // No one can edit approved or rejected leaves
    ...
    // If BOTH statuses are approved, cannot edit
    if (hodStatus === 'approved' && adminStatus === 'approved') {
      return false;
    }
    
    // If either status is rejected, cannot edit
    if (hodStatus === 'rejected' || adminStatus === 'rejected') {
      return false;
    }
    
    // Only pending leaves (where at least one is still pending) can be edited
